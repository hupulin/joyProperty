 ...
 */
public class UserInfoBulkHelper {

    protected static UserInfoBulkHelper instance;
    private UserInfoTo userInfoTo;
    private Context mContext;

    private UserInfoBulkHelper(Context context) {
         mContext = context;
        load(context);
    }


    public static UserInfoBulkHelper getInstance(Context context) {
        if (instance == null) {
            synchronized (UserInfoBulkHelper.class) {
                if (instance == null) {
                    instance = new UserInfoBulkHelper(context);
                }
            }
        }
    ...
    private void save() {
        ConfigUtil.saveString(PreferenceManager.getDefaultSharedPreferences(mContext),
                KeyValue.KEY_USER_INFO_BULK, JSON.toJSONString(userInfoTo));
    }

    public void load(Context context) {
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);
        String userJson = ConfigUtil.getString(sp, KeyValue.KEY_USER_INFO_BULK, "");
