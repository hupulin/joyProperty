...
public class SelectTaskServiceActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initView(List<TaskServiceTypeTo> typeTos) {
        ...
        for (int i=0;i<typeTos.size();i++){
            TaskServiceTypeTo typeTo=typeTos.get(i);
            ...
            nextIcon.setVisibility(typeTo.getWorkTypeInfoList() == null ? View.GONE : View.VISIBLE);
            view.setTag(typeTo);
            view.setOnClickListener(v -> {

                TaskServiceTypeTo serviceTypeTo = (TaskServiceTypeTo) v.getTag();
                allType=serviceTypeTo.getTypeName();
                allTypeSid=serviceTypeTo.getTypeApartmentSid();
                if (serviceTypeTo.getWorkTypeInfoList() != null) {

                    initView(serviceTypeTo.getWorkTypeInfoList());
                    ...
                    EventBus.getDefault().post(new RefreshEventOther(serviceTypeTo.getTypeApartmentSid(),3,serviceTypeTo.getTypeName(),categorySid));
                    finish();
    ...
    private void initViewOne() {
        ...
        View baseView=View.inflate(getThisContext(), R.layout.select_category_item_new, null);
...
//        baseView.findViewById(R.id.carInspectLayout).setOnClickListener(v -> setList("5CFB60A1-C1FC-4751-B123-05157F02C70D","违停管理"));
       baseView.findViewById(R.id.houseKeeping).setOnClickListener(v -> setList("B89C08B9-CD76-4B94-AE27-2617157180EF","日常工单"));
...
//        typeParam.setApartmentSid();
//
//        typeParam.setCategorySid(typeSid);
        Log.i("222", "setList: "+getIntent().getStringExtra("ApartmentSid")+"22222222"+typeSid);
        ...
        api.getWorkTypeByApartmentSid(getIntent().getStringExtra("ApartmentSid"),typeSid,new HttpCallback<MessageTo<List<TaskServiceTypeTo>>>(getThisContext()) {
            @Override
            public void success(MessageTo<List<TaskServiceTypeTo>> msg, Response response) {
                Log.i("222", "22222222222222: ");

                if (msg == null) return;
                if (msg.getSuccess() == 0) {
                    dialogFragment.dismissAllowingStateLoss();
                    Log.i("222", "setList: "+msg.getData().toString());
            ...
            public void failure(RetrofitError error) {
                Log.i("222", "22222222222222: "+error.toString());
