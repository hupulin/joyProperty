...
public class SelfShopActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initData(String index) {
        ...
        api.getMoreSelf(param, new HttpCallback<MessageToBulk<List<ShopListDetailTo>>>(getThisContext()) {
            @Override
    ...
    private void getSalesData(String index) {
        ...
        param.setPageSize("20");
        param.setCommunityId(mUserHelperBulk.getUserInfoTo().getApartmentSid());
        api.getBySaleSelf(param, new HttpCallback<MessageToBulk<List<ShopListDetailTo>>>(getThisContext()) {
            @Override
            public void success(MessageToBulk<List<ShopListDetailTo>> msg, Response response) {
                dialogFragment.dismiss();
                setListComplement();
    ...
    private void getNewGoods(String index) {
        ...
        param.setPageSize("20");
        param.setCommunityId(mUserHelperBulk.getUserInfoTo().getApartmentSid());
        api.getByNewGoodsSelf(param, new HttpCallback<MessageToBulk<List<ShopListDetailTo>>>(getThisContext()) {
            @Override
            public void success(MessageToBulk<List<ShopListDetailTo>> msg, Response response) {
                dialogFragment.dismiss();
                setListComplement();
    ...
    public void showType(){
        ...
        for(CategoryTo shopListTo:categoryList){
            ...
            linearLayout.setOnClickListener(v -> {
                startActivity(new Intent(getThisContext(),SelfShopActivity.class));
                finish();
            });
