...
public class NewGoodsDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void immediatelyBuy(String goodsNumber){
       ...
       {
           specificationsId=goodsDetailFragment.specificationsId;
           if (TextUtils.isEmpty(specificationsId)) {
    ...
    private void PurchaseDialogShow(boolean isAddCar) {
        ...
        purchaseAdd.setOnClickListener(view -> {
            ...
            purchaseSpecification.setText("已选择:" + specificationsName + "  x" + purchaseNumber.getText().toString());
        ...
        mFlowLayout.setAdapter(new TagAdapter<String>(mNames) {
            ...
            public View getView(FlowLayout parent, int position, String s) {
                ...
                mRelativeLayout.setOnClickListener(v -> {
                    ...
                    goodsDetailFragment.specificationContent.setText("已选择:" + specificationsName + "  x" + purchaseNumber.getText().toString());
                });
                ...
                if (specificationCount==0) {
                    ...
                    specificationsName = goodsSpecificationsList.get(0).getSpecificationsName();
                    purchaseSurplus.setText("库存:"+goodsDetail.getInventoryNum()+" 件");
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.purchase:
                if (!canAddOrPurchase)
                    return;
                if (TextUtils.isEmpty(goodsDetailFragment.specificationsId))
                PurchaseDialogShow(false);
                else
                    immediatelyBuy(goodsDetailFragment.purchaseNum);
                break;
            case R.id.add_car:
                if (!canAddOrPurchase)
                    return;
                if (TextUtils.isEmpty(goodsDetailFragment.specificationsId))
                    ...
                    addCar(goodsDetail.getGoodsId(),goodsDetailFragment.purchaseNum,goodsDetailFragment.specificationsName,goodsDetailFragment.specificationsId);
                break;
            case R.id.iv_back:
                finish();
                goToAnimation(2);
    ...
    public void setAddCarOrPurchase(){
        ...
        goodsDetailFragment.setImmediatelyPurchase(() -> {
            specificationsName=goodsDetailFragment.specificationsName;
            specificationsId=goodsDetailFragment.specificationsId;
            retailPrice=Double.parseDouble(goodsDetailFragment.retailPrice);
            if (TextUtils.isEmpty(goodsDetailFragment.specificationsId))
                PurchaseDialogShow(false);
            else
                immediatelyBuy(goodsDetailFragment.purchaseNum);
