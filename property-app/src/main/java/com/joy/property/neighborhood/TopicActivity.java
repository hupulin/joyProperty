 ...
 **/
public class TopicActivity extends BaseActivity implements NeighborPostToAdapter.INeighborPostTo, View.OnClickListener {
    ...
    private boolean canAddHeadView;
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        ...
        if (!isChange) {
            ...
            mAdapter.setOnNeighborPost(TopicActivity.this);
            setList(0);
    ...
    private void setList(final int index) {
            ...
            System.out.println(getIntent().getStringExtra("interactionSid") + "/" + mHelper.getSid() + "/" + mUserHelper.getSid());
            api.getInteractionDetail(getIntent().getStringExtra("interactionSid"), SpUtil.getBoolean(getThisContext(),"ParkNeighbor")?"":mHelper.getSid(), mUserHelper.getSid(), new HttpCallback<MessageTo<NeighborhoodInteractionTo>>(getThisContext()) {
                        ...
                        public void success(MessageTo<NeighborhoodInteractionTo> msg, Response response) {
                            if (msg.getSuccess() == 0) {
                                ...
                                canAddHeadView=true;
                                topicTitle = msg.getData().getActivityTitle();
                                if (msg.getData().getShowbtns() == 1) {

                                    confirmJoin.setVisibility(View.GONE);
                                }
                                if (msg.getData() != null && msg.getData().getNeighborPostToList() != null) {
                                    postList.clear();
                                    System.out.println(msg.getData()+"data");
                                    postList.addAll(msg.getData().getNeighborPostToList());
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.confirmJoin:
                ...
                SpUtil.put(getThisContext(), "TopicSid", getIntent().getStringExtra("interactionSid"));
                SpUtil.put(getThisContext(),"topicTitle",topicTitle);
                startActivity(intent);
                joinCampaign();
