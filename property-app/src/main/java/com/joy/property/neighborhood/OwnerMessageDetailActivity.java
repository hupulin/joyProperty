 ...
 * 邻居圈消息详情页
 */
public class OwnerMessageDetailActivity extends BaseActivity implements OnClickListener {
    ...
    private NeighborLikeTo mUserComment;
    private View view;
    ...
    private void initIntentDatas() {
        mUserComment = (NeighborLikeTo) getIntent().getSerializableExtra("mode");
    }
    ...
    private void initPopupWindow() {
        ...
        if( !SpUtil.getBoolean(getThisContext(), "isGood"))
            ...
            public void success(MessageTo<NeighborLikeTo> msg, Response response) {
                if (msg.getSuccess() == 0) {
                    ...
                    if (msg.getData() != null && msg.getData().getLikeOwner() != null) {
                        ...
                        int i = -1;
                   SpUtil.put(getThisContext(), "isGood", false);
                        for (NeighborLikeTo likeTo : list) {
                            i++;
                        ...
                        SpUtil.put(getThisContext(),"isGood",true);
                        ...
                        mTriangle.setVisibility(View.VISIBLE);
                        mFlImage.setVisibility(View.VISIBLE);
                        ImageView imageView = new ImageView(OwnerMessageDetailActivity.this);
                        imageView.setBackgroundResource(R.drawable.selector_good);
                        mFlImage.addView(imageView, 0, param);
                        int i = -1;
                        for (NeighborLikeTo likeTo : list) {
                            i++;
                            TextView textView = new TextView(OwnerMessageDetailActivity.this);
                            if (i < list.size() - 1) {
                                textView.setText(likeTo.getLikeOwner().getName() + ",");
                            } else {
                                textView.setText(likeTo.getLikeOwner().getName());
    ...
    protected Context getThisContext() {
        return OwnerMessageDetailActivity.this;
    }
    @Override
