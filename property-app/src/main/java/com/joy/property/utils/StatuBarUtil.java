    ...
    public static void setStatueBarBlueColor( Window window){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {//5.0 全透明实现
         ...
         //   window.setStatusBarColor(Color.WHITE);
             window.setStatusBarColor(Color.parseColor("#4fb2d6"));//calculateStatusColor(Color.WHITE, (int) alphaValue)
        }
    }
    //显示响应速度蓝色
    public static void setStatueBarReactionColor( Window window){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {//5.0 全透明实现
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            window.getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_STABLE|View.SYSTEM_UI_FLAG_VISIBLE);
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
         //   window.setStatusBarColor(Color.WHITE);
             window.setStatusBarColor(Color.parseColor("#000000"));//calculateStatusColor(Color.WHITE, (int) alphaValue)
        }
    }
    //显示状态栏白色


    public static void setStatueBarNormalBlueColor( Window window){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {//5.0 全透明实现
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            window.getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
                    | View.SYSTEM_UI_FLAG_LAYOUT_STABLE);
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            window.setStatusBarColor(Color.parseColor("#008cee"));//calculateStatusColor(Color.WHITE, (int) alphaValue)
        }
    }
