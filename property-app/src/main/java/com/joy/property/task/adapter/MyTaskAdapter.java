 ...
 * 记录类型（1：呼叫，2：物业响应，20：转发，21：退回，22:处理中
 *          3：撤消，4：处理完成，待评价，6：流程结束，9：关闭）
 * 以前的 状态 处理中 实际上 是保护  2开头的所有状态
 */
public class MyTaskAdapter extends ModeListAdapter<ServiceMainExpandTo> {
...
//        }
        ...
        if (!TextUtils.isEmpty(mode.getServiceCategory())) {
            ...
            if ("7D2B996C-12EC-4CD4-8499-B453E96AF11F".equals(mCategorySid)) {
                ...
                switch (state) {
                    case 1:
                        if (!"null".equals(mode.getServiceBookingTime())&&mode.getServiceBookingTime()!=null && StringUtils.isNotEmpty(mode.getServiceBookingTime()))
                        {
                            createOnTime=compare.BookingStrToDate(mode.getServiceBookingTime(), mode.getCreatedOn());
                ...
                switch (state) {
                    case 1:
                        ...
                        {
                            createOnTime=compare.BookingStrToDate(mode.getServiceBookingTime(), mode.getCreatedOn());
                            System.out.println(createOnTime+"第三个个");
                            if (compare.VaryRed2(createOnTime,mode.getReplyTime(), mode.getStartTime(), mode.getEndTime(), mode.getProcessCycle())) {
                ...
                switch (state) {
                    case 1:
                        ...
                        {
                            createOnTime=compare.BookingStrToDate(mode.getServiceBookingTime(), mode.getCreatedOn());
                            System.out.println(createOnTime+"第无个个");
                            if (compare.VaryRed2(createOnTime,mode.getReplyTime(), mode.getStartTime(), mode.getEndTime(), mode.getProcessCycle())) {
                ...
                switch (state) {
                    case 1:
                        if (mode.getServiceBookingTime()!=null && StringUtils.isNotEmpty(mode.getServiceBookingTime()))
                        {
                            ...
                            createOnTime=compare.BookingStrToDate(mode.getServiceBookingTime(), mode.getCreatedOn());
                            System.out.println(createOnTime+"第六个个");
                ...
                switch (state) {
                    case 1:
                        ...
                        {
                            createOnTime=compare.BookingStrToDate(mode.getServiceBookingTime(), mode.getCreatedOn());
                            System.out.println(createOnTime+"第七个个");
                            if (compare.VaryRed2(createOnTime,mode.getReplyTime(), mode.getStartTime(), mode.getEndTime(), mode.getProcessCycle())) {
    ...
    public  void getCompare(){
       ...
       if (mode.getStartTime()==null)
           mode.setStartTime(DateUtil.getDateString());
        if (mode.getEndTime()==null)
            mode.setEndTime(DateUtil.getDateString());
        //  优先使用预约时间 再使用 提报
        if (mode.getEndTime()==null||mode.getStartTime()==null)
            return;
        Date createOnTime=mode.getCreatedOn();
        DateFormat fmt= new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date reply=fmt.parse(replyTime, new ParsePosition(0));
//响应超时 超时大于15分钟
        System.out.println(createOn+responseTime+mode.getStartTime()+mode.getEndTime()+"===========");
        ...
        if (!"null".equals(mode.getServiceBookingTime())&&!TextUtils.isEmpty(mode.getServiceBookingTime()) && StringUtils.isNotEmpty(mode.getServiceBookingTime()))
        {
            createOnTime=compare.BookingStrToDate(mode.getServiceBookingTime(), mode.getCreatedOn());
