 ...
 */
public class CaredCenterAdapter extends ModeListAdapter<UserInfoTo> {
    private Context mContext;

    public CaredCenterAdapter(Context context) {
        super(context);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        CaredCenterCache holder;
        LayoutInflater inflater = LayoutInflater.from(mContext);
        if (row == null) {
            row = inflater.inflate(R.layout.list_item, null);
            holder = new CaredCenterCache(row);
            row.setTag(holder);
        } else {
            holder = (CaredCenterCache) row.getTag();
        }
